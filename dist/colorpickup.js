...
((window, document, Math, undefined) => {
      ...
      switch (key) {
        ...
        case 'components':
          if (typeof options.components === 'object') {
            settings.components = _extends({}, settings.components, options.components);
          }
    ...
    if (hasInstance) {
      ...
      for (let selector in instances) {
        if (element.matches(selector)) {
          ...
          const options = _extends({}, instances[selector]);
  ...
  function removeAllListeners() {
    eventRegistry.forEach((_ref) => {let context = _ref.context,type = _ref.type,listener = _ref.listener,options = _ref.options;
      if (context) {
  ...
  window.ColorPickUp = (() => {
    ...
    for (const key in methods) {
      ColorPickUp[key] = function () {for (var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++) {args[_key] = arguments[_key];}
